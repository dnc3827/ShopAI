...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <Link to={`/product/${product.id}`} className="group flex flex-col h-full">
      <article className="relative bg-white rounded-custom border border-slate-100 shadow-card hover:shadow-card transition-shadow duration-300 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex flex-col flex-1 p-4 gap-2 bg-white">
          {/* Category Badge */}
          <div>
            <span className="inline-flex items-center rounded-pill bg-surface px-2 py-0.5 text-xs font-medium text-slate-600 border border-slate-100">
              {product.categories?.name || 'Danh mục khác'}
            </span>
          </div>

          {/* Product Name */}
          <h3 className="text-md font-semibold text-slate-800 transition-colors group-hover:text-primary line-clamp-2 min-h-[56px] leading-normal">
            {product.name}
          </h3>

          {/* Description */}
          {product.description && (
            <p className="text-sm text-slate-500 line-clamp-2 mb-2 leading-normal">
